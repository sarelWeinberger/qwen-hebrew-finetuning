...
CLEANUP_RULES = [
    ...
    {
        'regex': (r'\b[A-Za-z0-9!#$%&\'+/=?^`{|}-]+(?:\.[A-Za-z0-9!#$%&\'+/=?^`{|}-]+)*@(?:(?:[A-Za-z0-9](?:[A-Za-z0-9-][A-Za-z0-9])?\.)+[A-Za-z0-9](?:[A-Za-z0-9-][A-Za-z0-9])?|\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[A-Za-z0-9-]*[A-Za-z0-9]:)\])\b', '<EMAIL>'),
        'bucket_name': 'gepeta-datasets',
        'path': f'partly-processed/{CLEAN_PATH}/PII/',
        'info': 'PII deletion – Comprehensive email pattern matching including special characters and IP-based email addresses.'
    },
    {
        'regex': (r'(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)', '<IP>'),
        'bucket_name': 'gepeta-datasets',
        'path': f'partly-processed/{CLEAN_PATH}/PII/',
